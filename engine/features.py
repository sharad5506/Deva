...
from shlex import quote  # Updated import for safe quotingimport re
import sqlite3
import struct
import subprocess
import time
...
# Load environment variables (make sure to call this once in your app initialization)
load_dotenv(dotenv_path=Path('engine\\config.py'))

# Set openai API key from environment variable
openai.api_key = os.getenv("OPENAI_API_KEY")

con = sqlite3.connect("deva.db")
cursor = con.cursor()
...
def hotword():
    ...
    try:
        access_key = "69W0bpzuokqtWYeH2QVMQp/h6KQ84l9kS5Q00wtEa3dk2CJYLJs6mg=="  # Replace with your Picovoice Console access key

        # Path to custom wake word keyword file (.ppn)
        keyword_path = "engine\\Hey Deva.ppn"  # Replace with actual path to your custom .ppn file
...
def whatsApp(mobile_no, message, flag, name):
    ...
    if flag == 'message':
        target_tab = 12
        deva_message = "message send successfully to "+name

    elif flag == 'call':
        target_tab = 7
        message = ''
        deva_message = "calling to "+name

    else:
        target_tab = 6
        message = ''
        deva_message = "staring video call with "+name
...
def chatBot(query):
    if not openai.api_key:
        speak("OpenAI API key is not set. Please set it in your environment.")
        return "API key missing"

    try:
        # Call OpenAI Chat API (GPT-4o-mini/GPT-3.5 Turbo)
...
def open_notepad_and_write():
    ...
    try:
        ...
        if command_text.startswith("open notepad and write"):
            text_to_write = command_text[len("open notepad and write "):]

            file_path = r"D://Deva//output.txt"

            # Write text to file at the specified path
            ...
            print("Command did not start with 'open notepad and write'")
